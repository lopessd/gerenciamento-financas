...
const kanbanColumns = [
  ...
  },
  {
    ...
    title: "Pendência",
    icon: FileText,
    color: "bg-orange-100/60 border-orange-200",
  ...
  },
  {
    ...
    title: "Finalizado",
    icon: CheckCircle,
    color: "bg-green-100/60 border-green-200",
...
export default function FechamentoKanban({ onViewFechamento }: FechamentoKanbanProps) {
  ...
  return (
    <Card className="border-0 shadow-md">
      {/* Use an inner container that mirrors the page padding so the filters align with page header */}
      <CardHeader className="px-0 py-2 sm:py-3">
        <div className="w-full px-2 sm:px-3 flex flex-col space-y-4">
          ...
          {/* Filtros */}
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 -mb-1 max-w-full overflow-hidden">
            {/* Busca */}
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <Select value={dateFilter} onValueChange={setDateFilter}>
                <SelectTrigger className="text-sm !h-9 sm:!h-10 !px-3 items-center w-auto sm:w-auto sm:min-w-[120px] flex-shrink-0">
              ...
              {kanbanColumns.map(column => {
                ...
                return (
                  <div key={column.id} className="flex flex-col overflow-hidden">
                    ...
                    {/* Cards da coluna */}
                    <div className={`${column.color} rounded-b-lg p-3 flex-1 min-h-[600px] overflow-hidden`}>
                      {items.length === 0 ? (
                        <div className="text-center text-muted-foreground text-sm py-8 min-h-[200px] flex items-center justify-center">
                          Nenhum item
                        </div>
                      ) : (
                        <div className="max-h-[650px] overflow-y-auto space-y-1.5 overflow-x-hidden">
